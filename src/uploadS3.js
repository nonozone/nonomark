...
const setHeaders = (request, values) => {
...
export const uploadWithXhr = ({
  ...
  if (typeof XMLHttpRequest !== 'function') {
    reject(new Error('XMLHttpRequest is unavailable. Provide a custom uploadRequest transport.'));
    return;
  }
  ...
  request.upload.addEventListener('progress', (event) => {
    ...
    else onProgress(event.loaded, Number(file?.size) || event.loaded);
  });
  request.addEventListener('load', () => {
    cleanup();
    if (request.status >= 200 && request.status < 300) resolve();
    else reject(new Error(`Image upload failed with HTTP ${request.status}.`));
  });
...
const requireUploadRequest = (value) => {
  if (!value || typeof value.uploadUrl !== 'string' || !value.uploadUrl || typeof value.publicUrl !== 'string' || !value.publicUrl) {
    throw new Error('getUploadRequest must return non-empty uploadUrl and publicUrl values.');
  }
  return value;
};

const addOptional = (asset, key, value) => {
  if (value !== undefined && value !== null && value !== '') asset[key] = value;
};
  ...
  return async (files, context = {}) => {
    ...
    for (const [index, file] of items.entries()) {
      ...
      await uploadRequest({
        ...
        headers: prepared.headers || (file.type ? { 'content-type': file.type } : undefined),
        file,
        signal,
        onProgress: report,
      ...
      const asset = {
        ...
        alt: prepared.alt || file.name.replace(/\.[^.]+$/, '') || 'image',
        provider: prepared.provider || defaultProvider,
        mimeType: file.type || undefined,
        size: Number(file.size) || undefined,
      };
      addOptional(asset, 'title', prepared.title);
      addOptional(asset, 'id', prepared.id);
      addOptional(asset, 'key', prepared.key);
      assets.push(asset);
    }
